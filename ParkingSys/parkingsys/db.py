...
def db_search(car_plate):
    ...
    c = db.cursor()
    cars = c.execute(
        'SELECT id, car_plate, checkin_time, checkout_time, "" as parking_fee'
        ' FROM cars'
...
def db_insert(car_plate, checkin_time):
    db = get_db()
    db.execute(
        'INSERT INTO cars (car_plate, checkin_time, checkout_time)'
        ' VALUES (?, ?, ?)',
        (car_plate, checkin_time, '')
    )
    db.commit()
...
def init_db(display=False, num=20):
    ...
    for i in range(CAR_NUM):
        ...
        checkin_time = str(in_time)
        checkout_time = ''
        if not (out_time is None):
            checkout_time = str(out_time)
    ...
    for i in range(CAR_NUM):
        ...
        if display:
            print('车牌号码：', car[0], '入场：', checkin_time, '出场:', checkout_time)
