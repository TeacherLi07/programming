...
def req_and_write(reqtext:str):
        ...
        response = client.run_ner_domain(request)
        ...
        # Write to file
        with open("./train-domain.txt", "a", encoding="utf-8") as file:
            file.write(bio_data)
...
def convert_to_bio(text, entities):
    ...
    for entity in entities:
        ...
        bio_annotations[start] = 'O' if mytag[entity['tag']] == 'O' else 'B-' + mytag[entity['tag']]
        for i in range(start + 1, end):
            bio_annotations[i] = 'O' if mytag[entity['tag']] == 'O' else 'I-' + mytag[entity['tag']]
    result = ''
